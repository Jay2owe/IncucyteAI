...
def json_clean(value):
    """Return strict JSON-compatible primitives without a custom encoder."""
...
class OutputFile:
    ...
    ``Auto-Organotypic/docs/scn-pipeline-plan.md`` and written the same way by
    ...
    #: the way ImageJ and Auto-Organotypic's ``scn_channel`` count, and describes
...
class ExportPlan:
    ...
    def to_dict(self):
        return json_clean({
            ...
            "new_scan_times": self.new_scan_times,
            "new_frame_count": self.new_frame_count,
...
class DownloadResult:
    ...
    def to_dict(self):
        status = ("cancelled" if self.cancelled else "error" if self.errors
                  else "written" if self.files else "empty")
        return json_clean({
            "status": status,
            "ok": self.ok,
...
__all__ = [
    ...
    "human_bytes", "derived", "microns_per_pixel", "json_clean",
