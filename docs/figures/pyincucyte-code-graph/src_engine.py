...
def authenticate(args):
    ...
    # Imported here because config imports this module for path and encryption
    # helpers. At call time both modules are fully initialized.
    from .config import ConfigStore

    store = ConfigStore()
    credentials = store.require(getattr(args, "host", None))
    if credentials.token_valid:
        return credentials.host, credentials.token
    token, expires_in = get_token(
        credentials.host, credentials.username,
        credentials.encrypted_password)
    credentials = credentials.with_token(token, expires_in)
...
def _download_single_image(host, token, item, state, state_lock,
